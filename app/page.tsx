                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="flexitarian">📿 Flexitarian (Select Veg Days)</SelectItem>
                      </SelectContent>
                    </Select>

                    {/* Interactive SVG Plate */}
                    <svg width="40" height="40" viewBox="0 0 60 60" className="flex-shrink-0 md:w-[60px] md:h-[60px]">
                      <circle
                        cx="30"
                        cy="30"
                        r="25"
                      ...
                      <circle
                        ...
                        strokeDasharray="4 2"
                      />
                      <text x="30" y="38" fontSize="20" textAnchor="middle" className="filter drop-shadow-md">
                        {profile.dietType === 'veg' ? '🥗' : profile.dietType === 'flexitarian' ? '📿' : '🍗'}
                      </text>
                    </svg>
                  </div>

                  {/* Day Selector for Flexitarian */}
                  {profile.dietType === 'flexitarian' && (
                    ...
                    >
                      <p className="text-xs font-semibold text-emerald-400 flex items-center gap-2">
                        ...
                        Kaunse din sirf Shakahari (Veg) khana hai?
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              const newDays = profile.nonVegDays?.includes(day)
                                ? profile.nonVegDays.filter((d) => d !== day)
                                : [...(profile.nonVegDays || []), day];
                              setProfile({ ...profile, nonVegDays: newDays });
                            }}
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${profile.nonVegDays?.includes(day)
                      ...
                      <p className="text-[10px] text-gray-500">
                        {profile.nonVegDays?.length === 0
                          ? 'Sare din Non-Veg dikhayega'
                          : `${profile.nonVegDays?.join(', ')} ko Veg meals dikhayenge`}
                      </p>
                    </motion.div>
                  )}
                </motion.div>
              ...
              >
                <Card className="glass backdrop-blur-xl bg-white/5 border-white/10">
                  <CardHeader>
                    ...
                    <CardDescription className="text-gray-400">
                      {profile.dietType === 'veg' ? '🥗 Pure Vegetarian' : profile.dietType === 'flexitarian' ? '📿 Flexitarian (Custom Days)' : '🍗 Non-Vegetarian'} • Card pe click karo benefits dekhne ke liye
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {/* Flexitarian Tabs */}
                    {profile.dietType === 'flexitarian' && results?.weeklyPlan && (
                      <div className="mb-6">
                        <div className="flex gap-2 p-1 rounded-lg bg-white/5 border border-white/10">
                          ...
                          >
                            📿 Religious Days ({profile.nonVegDays?.map(d => d.slice(0, 3)).join(', ') || 'None'})
                          </button>
                        </div>
